...
public class ai {
    ...
    /**
    * ai side
    * found an move and take the better
    ...
    **/
    public static void aiTurn(char[][] gameMatrix) {
        if (game.canBePlaced(gameMatrix, 1, 1)) gameMatrix[1][1] = 'O';
            ...
            int[][] lowGoodPrio = new int[8][]; // [] = line column diag [] = the number ex 2 in line
            int[][] lowBadPrio = new int[8][]; // [] = line column diag [] = the number ex 2 in line
    ...
    /**
     * found execute an move if only 1 case has empty in an line column give
    ...
    private static void urgentMove(char[][] gameMatrix, int j, int i) {
        if (j == 0) {
            ...
            int co = 0;
            do {
                if (game.canBePlaced(gameMatrix, co, co)) {
                    gameMatrix[co][co] = 'O';
                }
            ...
            } while (co < MATRIX_SIZE && i == 0 || co > 0 && i == 1);
        } else {
